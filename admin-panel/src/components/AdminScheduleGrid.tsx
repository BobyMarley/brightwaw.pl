...
const AdminScheduleGrid: React.FC = () => {
  ...
  const getOrderForSlot = (workerId: string, dayIndex: number, timeSlot: string) => {
    ...
    return orders.find(order => {
      ...
      return Math.abs((orderHour * 60 + orderMinute) - (slotHour * 60 + slotMinute)) < 30;
    });
  };
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '100%', overflowX: 'auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h1 style={{ margin: 0, color: '#1976d2' }}>График работников</h1>
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          <button onClick={() => navigateWeek(-1)} style={{ padding: '8px 16px', background: '#1976d2', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
            ← Предыдущая
          </button>
      ...
      <div style={{ background: 'white', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', overflow: 'hidden', minWidth: '800px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '150px repeat(7, 1fr)', background: '#f5f5f5', borderBottom: '2px solid #ddd' }}>
          <div style={{ padding: '15px 10px', fontWeight: 'bold', textAlign: 'center', borderRight: '1px solid #ddd' }}>Время</div>
          {weekDays.map((day, index) => {
            const date = getWeekDates()[index];
            return (
              <div key={day} style={{ padding: '10px', textAlign: 'center', borderRight: '1px solid #ddd' }}>
                <div style={{ fontWeight: 'bold', fontSize: '14px', marginBottom: '4px' }}>{day}</div>
        ...
        {workers.map(worker => (
          <div key={worker.id} style={{ borderBottom: '1px solid #eee' }}>
            <div style={{ background: '#fafafa', padding: '10px', borderRight: '1px solid #ddd', fontWeight: 'bold', fontSize: '14px', color: '#333' }}>
              {worker.name}
            </div>
