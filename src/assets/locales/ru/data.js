 async function fetchCardsData() {
    try {
      const response = await fetch('/assets/data/ru/priceData_ru.json?v=16');
  ...
  async function fetchTabsData(fileName) {
    try {
      const response = await fetch(`/assets/data/ru/${fileName}?v=16`);
      return await response.json();
    } catch (error) {
  ...
  async function createCardElement(cardId, initialType, cardData) {
    ...
    cardContent.innerHTML = `
      ...
      <section class="cards-input">
        ...
        <label>РЎС‚Р°РЅРґР°СЂС‚РЅР°СЏ СѓР±РѕСЂРєР°</label>
      </section>
      <section class="cards-input">
        <input type="radio" name="cleaning-type-${cardId}" data-card="${cardId}" data-item="1" value="generalna">
        <label>Р“РµРЅРµСЂР°Р»СЊРЅР°СЏ СѓР±РѕСЂРєР°</label>
      </section>
    `;
    ...
    try {
      ...
      radioInputs.forEach(input => {
        input.addEventListener("change", async () => {
          ...
          if (heading) {
            heading.textContent = selectedType === "kompleksowa" ? "Р§С‚Рѕ РІРєР»СЋС‡Р°РµС‚ СЃС‚Р°РЅРґР°СЂС‚РЅР°СЏ СѓР±РѕСЂРєР°" : "Р§С‚Рѕ РІРєР»СЋС‡Р°РµС‚ РіРµРЅРµСЂР°Р»СЊРЅР°СЏ СѓР±РѕСЂРєР°";
          }
  ...
  function initCleaningTypeButtons() {
    ...
    buttons.forEach(button => {
      button.addEventListener('click', async function() {
        ...
        const heading = document.getElementById('cleaning-type-heading');
        heading.textContent = type === 'kompleksowa' ? 'Р§С‚Рѕ Р’С…РѕРґРёС‚ РІ РЎС‚Р°РЅРґР°СЂС‚РЅСѓСЋ РЈР±РѕСЂРєСѓ' : 'Р§С‚Рѕ Р’С…РѕРґРёС‚ РІ Р“РµРЅРµСЂР°Р»СЊРЅСѓСЋ РЈР±РѕСЂРєСѓ';
      ...
      tabNumbers.forEach(tabNumber => {
        ...
        cleaningContent.innerHTML = `
          ...
          <div class="cleaning-image">
            <img src="${roomImages[tabNumber.toString()]}" alt="${tabDataItem.title || 'РЈР±РѕСЂРєР°'}" loading="lazy" style="width:100%;height:100%;object-fit:cover;border-radius:12px;" />
          </div>
        `;
      });
