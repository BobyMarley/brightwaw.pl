 async function fetchCardsData() {
    try {
      const response = await fetch('/assets/data/by/priceData_by.json?v=16');
  ...
  async function fetchTabsData(fileName) {
    try {
      const response = await fetch(`/assets/data/by/${fileName}?v=16`);
      return await response.json();
    } catch (error) {
  ...
  async function createCardElement(cardId, initialType, cardData) {
    ...
    cardContent.innerHTML = `
      ...
      <section class="cards-input">
        ...
        <label>РЎС‚Р°РЅРґР°СЂС‚РЅР°Рµ РїСЂС‹Р±С–СЂР°РЅРЅРµ</label>
      </section>
      <section class="cards-input">
        <input type="radio" name="cleaning-type-${cardId}" data-card="${cardId}" data-item="1" value="generalna">
        <label>Р“РµРЅРµСЂР°Р»СЊРЅР°Рµ РїСЂС‹Р±С–СЂР°РЅРЅРµ</label>
      </section>
    `;
    ...
    try {
      ...
      radioInputs.forEach(input => {
        input.addEventListener("change", async () => {
          ...
          if (heading) {
            heading.textContent = selectedType === "kompleksowa" ? "РЁС‚Рѕ СћРєР»СЋС‡Р°Рµ СЃС‚Р°РЅРґР°СЂС‚РЅР°Рµ РїСЂС‹Р±С–СЂР°РЅРЅРµ" : "РЁС‚Рѕ СћРєР»СЋС‡Р°Рµ РіРµРЅРµСЂР°Р»СЊРЅР°Рµ РїСЂС‹Р±С–СЂР°РЅРЅРµ";
          }
  ...
  function initCleaningTypeButtons() {
    ...
    buttons.forEach(button => {
      button.addEventListener('click', async function() {
        ...
        const heading = document.getElementById('cleaning-type-heading');
        if (heading) {
          heading.textContent = type === 'kompleksowa' ? 'РЁС‚Рѕ СћРєР»СЋС‡Р°Рµ СЃС‚Р°РЅРґР°СЂС‚РЅР°Рµ РїСЂС‹Р±С–СЂР°РЅРЅРµ' : 'РЁС‚Рѕ СћРєР»СЋС‡Р°Рµ РіРµРЅРµСЂР°Р»СЊРЅР°Рµ РїСЂС‹Р±С–СЂР°РЅРЅРµ';
        }
  ...
  function loadTabs(tabData) {
    try {
      ...
      const roomEmojis = {1: 'рџ›ЏпёЏ', 2: 'рџљЄ', 3: 'рџЌі', 4: 'рџ›Ѓ'};
        ...
        if (cleaningContent && tabDataItem) {
          cleaningContent.innerHTML = `
            ...
            <div class="cleaning-image">
              <img src="${roomImages[tabNumber.toString()]}" alt="${tabDataItem.title || 'РЈР±РѕСЂРєР°'}" style="width:100%;height:100%;object-fit:cover;border-radius:12px;" />
            </div>
          `;
        }
      });
