(function() {
    ...
    updateTotal() {
        ...
        // РЎРєРёРґРєР° 10% РїСЂРё Р·Р°РєР°Р·Рµ С‡РµСЂРµР· РєР°Р»СЊРєСѓР»СЏС‚РѕСЂ
        const discount = total * 0.1;
        const finalPrice = total - discount;
        
        const totalElement = document.getElementById('calcTotal');
        if (totalElement) {
            totalElement.innerHTML = `<span style="text-decoration: line-through; opacity: 0.5; font-size: 1.5rem;">${total} zЕ‚</span><br>${Math.round(finalPrice)} zЕ‚`;
        }
    },
    openModalWithCalc() {
