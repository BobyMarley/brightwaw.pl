 async function fetchCardsData() {
    try {
      const response = await fetch('/assets/data/pl/priceData_pl.json?v=5');
  ...
  async function fetchTabsData(fileName) {
    try {
      const response = await fetch(`/assets/data/pl/${fileName}?v=5`);
      return await response.json();
    } catch (error) {
    ...
    try {
      ...
      radioInputs.forEach(input => {
        input.addEventListener("change", async () => {
          ...
          if (heading) {
            heading.textContent = selectedType === "standardowe" ? "Co zawiera sprzД…tanie standardowe" : "Co zawiera sprzД…tanie generalne";
          }
